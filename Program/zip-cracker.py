...
Title(f"{CYAN}Zip Cracked{RESET}")
...
try:
    def ChooseZipRarFile():
        try:
            ...
            if sys.platform.startswith("win"):
                ...
                file = filedialog.askopenfilename(parent=root, title=f"Zip Cracked - Choose a file (.zip / .rar)", filetypes=[("ZIP/RAR files", "*.zip;*.rar"), ("ZIP files", "*.zip"), ("RAR files", "*.rar")])
            elif sys.platform.startswith("linux"):
                file = filedialog.askopenfilename(title=f"Zip Cracked - Choose a file (.zip / .rar)", filetypes=[("ZIP/RAR files", "*.zip;*.rar")])
            print(f"{BEFORE + current_time_hour() + AFTER} {INFO} File path: {CYAN + file + RESET}")
            return file
        except:
                ...
                with pyzipper.AESZipFile(file) as archive:
                    for filename in archive.namelist():
                        try:
                            ...
                            password_found += 1
                            print(f'{BEFORE + current_time_hour() + AFTER} {ADD} File: {CYAN + filename} {red}Password: {CYAN + password_test + RESET}')
                        except:
                            pass
            elif file.lower().endswith('.rar'):
                with rarfile.RarFile(file) as archive:
    ...
    def RandomCharacter(count):
        ...
        print(f"{BEFORE + current_time_hour() + AFTER} {WAIT} {CYAN}Brute force password cracking in progress.. (It can be long){RESET}")
        while password_found < count:
            Request()
    ...
    def WorldList():
        ...
        password_found = 0
        print(f"{BEFORE + current_time_hour() + AFTER} {INFO} Add more list in folder: {CYAN + path_folder_worldlist + RESET}")
        print(f"{BEFORE + current_time_hour() + AFTER} {WAIT} {CYAN}Password cracking by world list in progress.. (It can be long){RESET}")
    ...
    print(f"{BEFORE + current_time_hour() + AFTER} {CYAN}Starting Zip Cracked...{RESET}")
    file = ChooseZipRarFile()
    
    count = CountEncryptedFiles(file)
    print(f"{BEFORE + current_time_hour() + AFTER} {INFO} Number of files protected by password: {CYAN + str(count) + RESET}")
    if count == 0:
        Continue()
        Reset()

    Title(f"{CYAN}Zip Cracked - File: {file}{RESET}")
 ...
 """)
    ...
    method = input(f"{BEFORE + current_time_hour() + AFTER} {INPUT} Method -> {CYAN}")
